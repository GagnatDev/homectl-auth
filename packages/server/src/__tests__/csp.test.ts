...
// the registered apps' origins must be present in the form-action directive.
const app = createApp();
...
describe('Content-Security-Policy form-action', () => {
  ...
  it('allows the registered app origins so cross-origin login redirects are not blocked', async () => {
    ...
    for (const origin of TEST_APP.allowedOrigins) {
